...
export class MockSportsDataProvider implements SportsDataProvider {
  ...
  private oddsListeners: Map<string, Set<(odds: OddsSnapshot) => void>> =
    new Map();
  ...
  async getUpcomingMatches(limit: number = 10): Promise<Match[]> {
    ...
    return MOCK_MATCHES.filter(
      (m) => m.startTime > now && m.status === MatchStatus.SCHEDULED,
    ).slice(0, limit);
  }

  async getLiveMatches(): Promise<Match[]> {
    return MOCK_MATCHES.filter((m) =>
      [MatchStatus.LIVE, MatchStatus.HALFTIME].includes(m.status),
    );
  }
  ...
  private initializeOddsSimulation() {
    ...
    this.oddsUpdateInterval = setInterval(() => {
      MOCK_MATCHES.filter((m) => m.status === MatchStatus.LIVE).forEach(
        (match) => {
          ...
          if (currentOdds) {
            ...
            const newOdds: OddsSnapshot = {
              ...
              odds: {
                ...
                draw: (
                  parseFloat(currentOdds.odds.draw) *
                  (1 + (Math.random() - 0.5) * variation)
                ).toFixed(4),
                awayWin: (
