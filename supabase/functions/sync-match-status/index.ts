/**
 * sync-match-status — StatsPerform edition with auto-trigger settlement (Hedera)
 *
 * Identical to the statsperform-cre version EXCEPT: when FullTime is detected
 * for the first time (goalserve_finished flips from false → true), this function
 * also fires settle-match-hedera as a fire-and-forget POST.
 *
 * This enables fully automatic settlement on the Hedera testnet branch without
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const dbMatch of dbMatches) {
      ...
      // ── Detect FullTime for the first time → set flag + auto-trigger settle ─
      ...
      // ── Auto-trigger settle-match-hedera when FullTime is first detected ──
      if (justBecameFullTime) {
        console.log(
          `[sync-hedera] FullTime detected for ${dbMatch.id} (SP: ${dbMatch.statsperform_match_id}) — auto-triggering settle-match-hedera`,
        );

        // Fire-and-forget: settle-match-hedera fetches SP data itself (auto-fetch mode)
        fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/settle-match-hedera`, {
          method: "POST",
          headers: {
            apikey: Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!,
            Authorization: `Bearer ${Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")}`,
          ...
          console.error("[sync-hedera] settle-match-hedera fire-and-forget error:", err);
        });
